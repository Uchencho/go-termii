...
func NewClientConfig() (termii.Config, *http.Client) {
	httpClient := http.Client{}

	termiiConfig := termii.Config{
		APIKey:   termiiTestApiKey,
		BaseURL:  termiiBaseUrl,
		SenderID: termiiSenderID,
	}

	return termiiConfig, &httpClient
}
...
func TestSendTokenSuccess(t *testing.T) {
	...
	termiiClient, termiiHttpClient := NewClientConfig()
	c := termii.NewClient(termiiClient, termiiHttpClient)
